/**
 * Golden Answer Test Runner — Troubleshooting (Non-Legal) Documents
 *
 * Runs each question from golden_ts_guide_tests.json through the full KTS
 ...
 *
 * Usage: Register as `kts.runTSGoldenTests` command in extension.js then:
 *   Cmd+Shift+P → "KTS: Run TS Golden Tests"
 */
const fs = require('fs');
...
function loadTests() {
  const testsPath = path.join(__dirname, 'golden_ts_guide_tests.json');
 ...
 * @param {object} test    - One element from golden_ts_guide_tests.json
...
async function runSingleTest(vscode, test, { outputChannel, shared, sourcePath }) {
  ...
  // Generate answer via LLM — uses KTS (non-legal) system prompt
  let answerText = '';
  let modelId = 'none';
  try {
    ...
    if (model) {
      ...
      // Non-legal / troubleshooting system prompt
      const systemPrompt = KTS_SYSTEM_PROMPT;
...
/**
 * Run all golden TS guide tests and save results to disk.
 *
 ...
 */
async function runTSGoldenTests(vscode, outputChannel, shared) {
  const testSourcePath = shared.testSourcePath;
  if (!testSourcePath) {
    throw new Error('runTSGoldenTests: shared.testSourcePath is required. The command handler must provide it via folder picker.');
  }
  ...
  outputChannel.appendLine(`\n=== TS Guide Golden Answer Tests: ${tests.length} questions ===\n`);
  ...
  // Save results
  const resultsDir = path.join(__dirname, 'golden_ts_guide_results');
...
module.exports = { runTSGoldenTests, runSingleTest, loadTests };
