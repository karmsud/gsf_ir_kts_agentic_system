/**
 * Golden TS Guide Scorer — LLM-as-Judge for Troubleshooting Documents
 *
 * Scores each TS Guide golden test answer on 5 dimensions using a Copilot LLM.
...
// ---------------------------------------------------------------------------
// Weights for composite score (same as PSA)
// ---------------------------------------------------------------------------
const WEIGHTS = {
  completeness: 0.25,
  accuracy: 0.30,
...
// ---------------------------------------------------------------------------
// Judge prompt — adapted for troubleshooting / non-legal documents
// ---------------------------------------------------------------------------
function buildJudgePrompt(test, actualAnswer) {
  return `You are an expert judge evaluating the quality of a RAG system's answer about a technical troubleshooting guide (HP Business Desktop Troubleshooting Guide).
...
/**
 * Score an array of TS golden test results using LLM-as-judge.
 *
 * @param {object}   vscode    - The vscode API
 * @param {object}   model     - VS Code LanguageModel to use as judge
 * @param {object[]} results   - Array from golden_ts_guide_runner
...
async function scoreResults(vscode, model, results, outputChannel, options = {}) {
  ...
  let baseline = null;
  const baselinePath = options.baselinePath || path.join(__dirname, 'golden_ts_guide_baseline.json');
...
function saveScores(scores, resultsDir) {
  ...
  const outputPath = path.join(resultsDir || __dirname, 'golden_ts_guide_results', `${timestamp}_scores.json`);
...
function saveAsBaseline(scores) {
  const baselinePath = path.join(__dirname, 'golden_ts_guide_baseline.json');
